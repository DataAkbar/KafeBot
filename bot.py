...
async def haydovchi(message: types.Message):
    await message.answer(f"Bugunga nimalarni zaqaz berishni istaysiz?", reply_markup=submenu)


@dp.message_handler(Text(equals="Pitsa"))
async def toshkents(message: types.Message):
    await message.answer(f"Siz qaysi tumanda turibsiz?\nTumanlardan birini tanlang", reply_markup=url)
